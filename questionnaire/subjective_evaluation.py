...
class SUBJECTIVE_EVALUATION:
	...
	def SE(self):
		...
		def p(n):
			self.q = [self.q1.get(),self.q2.get(),self.q3.get(),self.q4.get()]
		...
		self.s2 = Scale(self.root, orient = 'h',
				...
				command = p, showvalue=False)
		self.l3 = Label(self.root,text='Q3. 相手の動きをどれだけ理解できましたか？')
		...
		self.s3 = Scale(self.root, orient = 'h',
				...
				command = p, showvalue=False)
		self.l4 = Label(self.root,text='Q4. ロボットの動きをどれだけ理解できましたか？')
		...
		self.button_e.grid(column=0,columnspan=2,row=13,pady=10)
